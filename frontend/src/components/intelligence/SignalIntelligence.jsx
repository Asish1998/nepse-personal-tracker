...
const SIGNALS = [
  ...
  { sym: 'CIT', signal: 'SQUEEZE', confidence: 'HIGH', reason: 'Bollinger Bands Squeezed for 12 days. Extreme low volatility detected.', price: 2150 },
  ...
  { sym: 'CHDC', signal: 'SQUEEZE', confidence: 'MED', reason: 'BB Squeeze on Weekly chart. Long term accumulation phase.', price: 1040 },
    ...
    <div style={styles.container}>
      <div style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <span style={styles.badge}>VOLATILITY & TREND CONFLUENCE</span>
        </div>
        <div style={styles.timer}>Scanning Market Depth...</div>
      </div>
          ...
          <div key={s.sym} className="card" style={styles.signalCard}>
            ...
            <div style={styles.reasonBox}>
              ...
              <div style={styles.reasonText}>
                {s.signal === 'SQUEEZE' ? <span style={styles.squeezeIcon}>🎯 </span> : null}
                {s.reason}
              </div>
            </div>
      ...
      <div style={styles.disclaimer}>
        ⚠️ Bollinger Band Squeezes indicate high probability of explosive movement. Ensure you wait for the breakout candle before entry.
      </div>
    </div>
  )
}
...
const styles = {
  ...
  signalCard: { padding: '20px', border: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: '16px', background: 'var(--bg-main)' },
  ...
  signalBadge: { padding: '6px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: '800', letterSpacing: '0.05em' },
  ...
  SQUEEZE: { background: 'rgba(245, 158, 11, 0.15)', color: '#d97706' },
  reasonBox: { flex: 1 },
  ...
  squeezeIcon: { color: '#d97706', fontSize: '14px' },
  cardBottom: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '16px', borderTop: '1px solid var(--border-subtle)' },
