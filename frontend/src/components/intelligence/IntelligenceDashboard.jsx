...
export default function IntelligenceDashboard() {
  ...
  const generateAIReport = async () => {
    ...
    try {
      ...
      const prompt = `Act as a trading psychology coach. Analyze this trader's journal. TRADE JOURNAL DATA:\n${tradeData}`
    ...
    <div style={styles.container}>
      <div style={styles.header}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <button style={styles.aiBtn} onClick={generateAIReport} disabled={loading}>
            {loading ? '🧠 Processing...' : '✨ Generate Deep AI Report'}
          </button>
        </div>
      </div>
      ...
      {aiReport && (
        <div style={styles.aiReportBox}>
          ...
          <div style={styles.aiReportContent}>{aiReport}</div>
        </div>
      )}
      ...
      <div className="intelligence-grid" style={styles.grid}>
        <Section title="Trade Intelligence" emoji="⚡" metrics={[
          ...
          { title: 'Risk/Reward', value: `${tradeInt.riskReward.toFixed(2)}x`, sub: `Avg Win: ${fmtNPR(tradeInt.avgProfit)}` },
          { title: 'Total Trades', value: tradeInt.totalTrades, sub: 'Completed cycles' }
        ]} insights={tradeInt.insights} />

        <Section title="Portfolio Intelligence" emoji="🛡️" metrics={[
        ...
        <Section title="Stock & Sector Intelligence" emoji="📊" metrics={[
          { title: 'Tracked Symbols', value: stockInt.stocks.length, sub: 'Over all time' },
          { title: 'Best Performer', value: stockInt.stocks[0]?.sym || '-', sub: stockInt.stocks[0] ? `NPR ${fmtNPR(stockInt.stocks[0].totalPL)}` : '', colorClass: 'profit' },
          { title: 'Worst Performer', value: stockInt.stocks[stockInt.stocks.length-1]?.sym || '-', sub: stockInt.stocks[stockInt.stocks.length-1] ? `NPR ${fmtNPR(stockInt.stocks[stockInt.stocks.length-1].totalPL)}` : '', colorClass: 'loss' }
...
const styles = {
  container: { padding: '24px 32px', maxWidth: '1400px', margin: '0 auto', paddingBottom: '100px' },
  ...
  sectionWrap: { background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '24px', boxShadow: 'var(--shadow-card)', position: 'relative', overflow: 'hidden' },
  sectionHeader: { display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' },
  sectionEmoji: { fontSize: '24px', background: 'rgba(255,255,255,0.05)', padding: '8px', borderRadius: '8px', display: 'inline-flex' },
  sectionTitle: { fontSize: '20px', fontWeight: '700', color: 'var(--text-main)' },
  metricsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '28px' },
  metricCard: { background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', justifyContent: 'center' },
  metricTitle: { fontSize: '12px', textTransform: 'uppercase', fontWeight: '700', color: 'var(--text-muted)', letterSpacing: '0.05em', marginBottom: '8px' },
  metricValue: { fontSize: '24px', fontWeight: '800', fontFamily: 'var(--mono)', color: 'var(--text-main)' },
  metricSub: { fontSize: '11px', color: 'var(--secondary)', marginTop: '6px', fontWeight: '600' },
  insightTitle: { fontSize: '14px', fontWeight: '700', color: 'var(--text-muted)', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid var(--border)', paddingBottom: '8px' },
  insightsList: { display: 'flex', flexDirection: 'column', gap: '12px' },
  aiBtn: { background: 'linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)', color: 'white', border: 'none', borderRadius: '12px', padding: '12px 24px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(139, 92, 246, 0.3)', transition: 'transform 0.2s' },
  aiReportBox: { background: 'rgba(139, 92, 246, 0.05)', border: '1px solid rgba(139, 92, 246, 0.2)', borderRadius: '16px', padding: '24px', marginBottom: '32px' },
  aiReportHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '16px', fontWeight: '800', color: '#a78bfa', borderBottom: '1px solid rgba(139, 92, 246, 0.2)', paddingBottom: '16px', marginBottom: '16px' },
  aiReportContent: { fontSize: '14px', lineHeight: '1.6', color: 'var(--text-main)' }
}
