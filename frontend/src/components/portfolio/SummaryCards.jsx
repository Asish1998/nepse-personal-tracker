...
export default function SummaryCards() {
  ...
  const unrealized = state.holdings.reduce(
    (acc, h) => {
      const { totalCost, fees } = effectiveBuyCost(h.qty, h.buy)
  ...
  const cards = [
    { label: 'Invested',        value: `NPR ${fmtNPR(unrealized.invested)}` },
    { label: 'Current Value',   value: `NPR ${fmtNPR(unrealized.value)}` },
    { label: 'Unrealized P/L',  value: `${unrealizedPL >= 0 ? '+' : ''}NPR ${fmtNPR(unrealizedPL)}`, color: unrealizedPL >= 0 ? 'var(--profit)' : 'var(--loss)' },
    { label: 'Realized P/L',    value: `${realized.profit >= 0 ? '+' : ''}NPR ${fmtNPR(realized.profit)}`, color: realized.profit >= 0 ? 'var(--profit)' : 'var(--loss)' },
    { label: 'Total Net P/L',   value: `${totalPL >= 0 ? '+' : ''}NPR ${fmtNPR(totalPL)}`, color: totalPL >= 0 ? 'var(--profit)' : 'var(--loss)', bold: true },
